...
PaletteFrame::PaletteFrame(QWidget* parent, const QString& name,
                           const QString& closeKey,
  ...
  switch (event->type()) {
    case QEvent::KeyPress: {
      ...
      switch (keyEvent->key()) {
        case Qt::Key_Down:
        case Qt::Key_Up: {
          /* We manually process Up/Down key to handle corner cases and for ease
           * of edits */
        ...
        case Qt::Key_PageUp:
          /* Forward QKeyEvent for PageDown/PageUp key since QAbstractListView
           * already implemented the procedure */
          event->ignore();
...
void PaletteFrame::showEvent(QShowEvent* event) { searchbox_->setFocus(); }
