...
PYBIND11_MODULE(__palette__, m) {
  ...
  m.def(
      "show_palette",
      [](PyPalette& palette, std::string closeKey) -> bool {
        show_palette(
            palette.name(), palette.placeholder(), palette.actions(),
            QString::fromStdString(closeKey), [](const Action& action) {
              py::gil_scoped_acquire gil;

              try {
                py::object trigger_action_py =
                    py::module::import("__palette__").attr("execute_action");
                ...
                auto write =
                    py::module::import("sys").attr("stdout").attr("write");
                write(error.what());
                write("\n");
                PyErr_Clear();
              }
...
void init_python_module() {
  ...
  _PyImport_FixupExtensionObject(ptr, str.ptr(),
                                 str.ptr()
