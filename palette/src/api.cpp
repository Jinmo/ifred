...
void show_palette(const QString& name, const QString& placeholder,
  ...
  postToThread2([=]() {
    ...
    g_current_widget->show(name, placeholder, actions, closeKey, func);
  });
}

void cleanup_palettes() { Q_CLEANUP_RESOURCE(theme_bundle); }

void set_path_handler(pathhandler_t handler) { pluginPath = handler; }
